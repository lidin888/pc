...
int main(int argc, char *argv[]) {
  ...
  printf("\nApplying openpilot-first profile (disable onboard smoothing) ...\n");

  if (check("WitSetBandwidth(256Hz)", WitSetBandwidth(BANDWIDTH_256HZ)) != 0) return 3;
  usleep(50000);
  if (check("WitWriteReg(ACCFILT, 0)", WitWriteReg(ACCFILT, 0)) != 0) return 3;
  usleep(50000);
  if (check("WitWriteReg(GYROFILT, 0)", WitWriteReg(GYROFILT, 0)) != 0) return 3;
  usleep(50000);
  if (check("WitSetOutputRate(100Hz)", WitSetOutputRate(RRATE_100HZ)) != 0) return 3;
  usleep(50000);
  if (check("WitSetContent(ACC|GYRO)", WitSetContent(RSW_ACC | RSW_GYRO)) != 0) return 3;
  usleep(50000);

  if (check("WitSaveParameter", WitSaveParameter()) != 0) return 4;
  usleep(200000);

  printf("\nDone. Persisted openpilot-first profile to module flash.\n");
  printf("  BANDWIDTH = 256Hz\n");
  printf("  ACCFILT = 0\n");
  printf("  GYROFILT = 0\n");
  printf("  RRATE = 100Hz\n");
  printf("  RSW = ACC|GYRO\n");
