...
class CarrotPlanner:
  ...
  def update(self, sm, v_cruise_kph, mode):
    ...
    self.events = EventsSP()
    carstate = sm['carState']
    vCluRatio = getattr(carstate, 'vCluRatio', 1.0)  # may not exist in Sunnypilot CarState
    #controlsState = sm['controlsState']
    radarstate = sm['radarState']
    model = sm['modelV2']

    #self.soft_hold_active = sm['carControl'].hudControl.softHoldActive # carrot 1
    self.soft_hold_active = getattr(sm['carState'], 'softHoldActive', 0)  # carrot 2 — field may not exist in Sunnypilot
