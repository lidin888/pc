...
class CarrotMan:
  def __init__(self):
    ...
    self.sm = messaging.SubMaster(['deviceState', 'carState', 'controlsState', 'longitudinalPlan', 'modelV2', 'selfdriveState', 'carControl', 'navRoute', 'liveLocationKalman', 'navInstruction', 'radarState'])
  ...
  def get_data_amap_navi(self):
    ...
    # 传递地图红绿灯数据
    self.carrot_serv.map_traffic_state = self.amap_navi.shared_data.map_traffic_state
    self.carrot_serv.map_traffic_countdown = self.amap_navi.shared_data.map_traffic_countdown
    self.carrot_serv.map_traffic_time = self.amap_navi.shared_data.map_traffic_time
  ...
  def make_send_message(self):
    msg = {}
    msg['Carrot2'] = self.sys_params.get("Version") or ''
  ...
  def make_tmux_data(self):
    try:
      ...
      subprocess.run(BASEDIR + "/selfdrive/apilot.py", shell=True, capture_output=True, text=False)
    except Exception as e:
  ...
  def send_tmux(self, ftp_password, tmux_why, send_settings=False):
    ...
    directory = git_branch + " " + car_selected + " " + (Params().get("DongleId") or '')
  ...
  def carrot_panda_debug(self):
    ...
    while True:
      if self.show_panda_debug:
        self.show_panda_debug = False
        try:
          debug_script = BASEDIR + "/selfdrive/debug/debug_console_carrot.py"
          if os.path.exists(debug_script):
            subprocess.run(debug_script, shell=True)
          else:
            print(f"debug_console_carrot.py not found, skipping")
        except Exception as e:
  ...
  def save_toggle_values(self):
    try:
      # Collect all params into a toggle values dict
      toggle_values = {}
      params = Params()
      for key in ["IsMetric", "IsLdw", "OpkrEnableDriverMonitoring"]:
        try:
          val = params.get(key)
          if val is not None:
            toggle_values[key] = val
        except Exception:
          pass
      file_path = os.path.join(BASEDIR, 'toggle_values.json')
