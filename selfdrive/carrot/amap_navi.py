...
class SharedData:
  def __init__(self):
    ...
    # Map-based traffic light data (from phone navigation app)
    self.map_traffic_state = 0       # 0=none, 1=red, 2=green, 3=left-turn green
    self.map_traffic_countdown = 0   # countdown seconds
    self.map_traffic_time = 0.0      # timestamp of last update
...
class AmapNaviServ:
  ...
  def lidar_object_blind(self):
    ...
    if self.dynamicBlindRange >= 1:
      ...
      laneWidthLeft = round(getattr(meta, 'laneWidthLeft', 0.0), 1)
      laneWidthRight = round(getattr(meta, 'laneWidthRight', 0.0), 1)
  ...
  # ----------------------
  # 更新地图红绿灯数据（高德等导航APP提供的信号灯+倒计时）
  # ----------------------
  def _update_traffic_light(self, json_obj):
    """Handle map-based traffic light data from phone navigation app.

    Supported JSON formats:
      {"trafficLight": {"state": 1, "countdown": 25}}
      {"trafficLight": {"state": 2, "countdown": 10}}
    Where state: 0=none, 1=red, 2=green, 3=left-turn green

    Also supports command format:
      {"cmd": "TRAFFIC", "arg": "state,countdown", "index": N}
    """
    # Format 1: direct JSON field
    if "trafficLight" in json_obj:
      ...
      with lock:
        ...
        self.shared_data.map_traffic_time = time.time()
      return

    # Format 2: command-based (TRAFFIC command)
  ...
  def make_navi_message(self):
    ...
    if self.sm.alive['modelV2']:
      ...
      if isOnroad:
        msg['l_lane_width'] = round(getattr(meta, 'laneWidthLeft', 0.0), 1)
        msg['r_lane_width'] = round(getattr(meta, 'laneWidthRight', 0.0), 1)
        msg['l_edge_dist'] = round(getattr(meta, 'distanceToRoadEdgeLeft', 0.0), 1)
        msg['r_edge_dist'] = round(getattr(meta, 'distanceToRoadEdgeRight', 0.0), 1)
        msg['atc_state'] = getattr(meta, 'laneChangeState', type('', (), {'raw': 0})()).raw
