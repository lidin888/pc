...
class CarrotServ:
  def __init__(self):
    ...
    # Map-based traffic light (from phone navigation app like 高德地图)
  ...
  def update_navi(self, remote_ip, sm, pm, vturn_speed, coords, distances, route_speed):
    ...
    if sm.alive['carState'] and sm.alive['selfdriveState']:
      ...
      v_ego = CS.vEgo
      v_ego_kph = v_ego * 3.6
      # distanceTraveled not available in Sunnypilot's SelfdriveState, compute from velocity
      delta_dist = v_ego * 0.05  # approximate: v_ego * dt (20Hz → 0.05s)
      self.totalDistance += delta_dist
      speedLimit = getattr(CS, 'speedLimit', 0)
      if speedLimit > 0 and self.active_carrot <= 1:
        self.nRoadLimitSpeed = speedLimit
    else:
      v_ego = v_ego_kph = 0
      delta_dist = 0
      CS = None
    ...
    # Map-based traffic light takes priority over DETECT (camera) data
    # If map data is fresh (within 5 seconds), use it
    map_traffic_age = time.time() - self.map_traffic_time
    if self.map_traffic_state > 0 and map_traffic_age < 5.0:
      msg.carrotMan.trafficState = self.map_traffic_state
      msg.carrotMan.trafficCountdown = self.map_traffic_countdown
    elif self.traffic_state > 0:
      msg.carrotMan.trafficState = self.traffic_state
      msg.carrotMan.trafficCountdown = 0
    else:
      msg.carrotMan.trafficCountdown = 0
    ...
    msg.carrotMan.szGoalName = self.szGoalName
    msg.carrotMan.szTBTMainTextNext = self.szTBTMainTextNext
    msg.carrotMan.szNearDirName = self.szNearDirName
    ...
    if self.active_carrot > 1 and self.active_kisa_count <= 0:
      ...
      # Copy fields individually since NavInstruction and NavInstructionCarrot are different types
      nav = sm['navInstruction']
      instruction = inst.navInstructionCarrot
      instruction.distanceRemaining = nav.distanceRemaining
      instruction.timeRemaining = nav.timeRemaining
      instruction.timeRemainingTypical = nav.timeRemainingTypical
      instruction.speedLimit = nav.speedLimit
      instruction.maneuverDistance = nav.maneuverDistance
      instruction.maneuverPrimaryText = nav.maneuverPrimaryText
      instruction.maneuverSecondaryText = nav.maneuverSecondaryText
      instruction.maneuverType = nav.maneuverType
      instruction.maneuverModifier = nav.maneuverModifier
      try:
        maneuvers = [{'distance': m.distance, 'type': m.type, 'modifier': m.modifier} for m in nav.allManeuvers]
        instruction.allManeuvers = maneuvers
      except Exception:
        pass
  ...
  def update(self, json):
    ...
    if "nRoadLimitSpeed" in json:
      ...
      self.szTBTMainTextNext = json.get("szTBTMainTextNext", "")
