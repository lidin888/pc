...
class Sidebar(Widget):
  def __init__(self):
    ...
    self._temp_status = MetricData("温度", "良好", Colors.GOOD)
    self._panda_status = MetricData("车辆", "在线", Colors.GOOD)
    self._connect_status = MetricData("连接", "离线", Colors.WARNING)
  ...
  def _update_temperature_status(self, device_state):
    ...
    if thermal_status == ThermalStatus.green:
      self._temp_status.update("温度", "良好", Colors.GOOD)
    elif thermal_status == ThermalStatus.yellow:
      self._temp_status.update("温度", "正常", Colors.WARNING)
    else:
      self._temp_status.update("温度", "高温", Colors.DANGER)

  def _update_connection_status(self, device_state):
    last_ping = device_state.lastAthenaPingTime
    if last_ping == 0:
      self._connect_status.update("连接", "离线", Colors.WARNING)
    elif time.monotonic_ns() - last_ping < 80_000_000_000:  # 80 seconds in nanoseconds
      self._connect_status.update("连接", "在线", Colors.GOOD)
    else:
      self._connect_status.update("连接", "错误", Colors.DANGER)

  def _update_panda_status(self):
    if ui_state.panda_type == log.PandaState.PandaType.unknown:
      self._panda_status.update("无", "熊猫", Colors.DANGER)
    else:
      self._panda_status.update("车辆", "在线", Colors.GOOD)
